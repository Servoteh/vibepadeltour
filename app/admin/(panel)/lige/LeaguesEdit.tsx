...
import { updateLeague, deleteLeague, type ActionState } from "@/app/admin/actions";
...
function LeagueRow({ l }: { l: EditLeague }) {
  ...
  return (
    <div className="space-y-2 rounded-2xl border border-navy/8 bg-paper p-4 shadow-[var(--shadow-soft)]">
      ...
      <form action={deleteLeague} className="border-t border-navy/8 pt-2">
        <input type="hidden" name="league" value={`${l.clubId}:${l.id}`} />
        <button className="text-xs font-medium text-red-600 hover:underline">
          Obriši ligu (briše grupe, parove, kola, raspored)
        </button>
      </form>
    </div>
  );
